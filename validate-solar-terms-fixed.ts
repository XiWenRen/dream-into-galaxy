/**
 * Solar Terms Validation Script for 2026 - Fixed Version
 * Correctly handles solar year boundary (terms span across calendar years)
 */
...
// Search across solar year (from spring equinox to next spring equinox)
function getSolarTermTimestampSolarYear(springEquinoxTs: number, targetLongitude: number): number {
  const startDays = (springEquinoxTs - J2000_TIMESTAMP) / 86400000;
  const endDays = startDays + 366; // Search up to 366 days after spring equinox
...
// Corrected astronomical almanac reference values for solar year 2026
// (from spring equinox 2026 to spring equinox 2027)
const REFERENCE_SOLAR_YEAR_2026: Record<number, string> = {
  ...
  285: "2027-01-05 20:30", // 小寒 (in 2027)
  300: "2027-01-20 14:03", // 大寒 (in 2027)
  315: "2027-02-04 08:30", // 立春 (in 2027)
  330: "2027-02-18 04:32", // 雨水 (in 2027)
  345: "2027-03-05 02:40", // 惊蛰 (in 2027)
};
...
console.log("=== Solar Terms Validation for Solar Year 2026 ===\n");
console.log("(From Spring Equinox 2026 to Spring Equinox 2027)\n");

// First find spring equinox 2026
const springEquinox2026 = getSolarTermTimestamp(2026, 0);
console.log(`Spring Equinox 2026 computed: ${formatDate(springEquinox2026)}`);
console.log(`Reference: ${REFERENCE_SOLAR_YEAR_2026[0]}`);
console.log();
...
for (const term of SOLAR_TERMS_LIST) {
  const ts = getSolarTermTimestampSolarYear(springEquinox2026, term.angle);
  const computedStr = formatDate(ts);
  const refStr = REFERENCE_SOLAR_YEAR_2026[term.angle];
...
// Verify solar longitude at equinoxes/solstices
console.log(`\n=== Solar Longitude at Key Dates ===`);
const keyDates = [
  { date: "2026-03-20", desc: "Spring Equinox", expected: 0 },
  { date: "2026-06-21", desc: "Summer Solstice", expected: 90 },
  { date: "2026-09-23", desc: "Autumn Equinox", expected: 180 },
  { date: "2026-12-21", desc: "Winter Solstice", expected: 270 },
];
...
for (const kd of keyDates) {
  ...
  const diff = longitudeDiff(lon, kd.expected);
  console.log(`  ${kd.date} (${kd.desc}): longitude = ${lon.toFixed(4)}°, expected ${kd.expected}°, diff = ${diff.toFixed(4)}°`);
}

// Check engine API behavior: getSolarTermTimestamp(2026, 285) returns Jan 2026
// which is the 小寒 of 2025 solar year, not 2026 solar year
console.log(`\n=== API Behavior Check ===`);
const xiaohan2026Calendar = getSolarTermTimestamp(2026, 285);
console.log(`getSolarTermTimestamp(2026, 285) = ${formatDate(xiaohan2026Calendar)}`);
console.log(`This is the 小寒 occurring in calendar year 2026 (part of 2025 solar year)`);
console.log(`The 小寒 for solar year 2026 occurs in 2027: ${REFERENCE_SOLAR_YEAR_2026[285]}`);

// Metrics (excluding the 5 cross-year terms for fair comparison, then including all)
const withinYearResults = results.filter(r => r.angle <= 270);
const crossYearResults = results.filter(r => r.angle > 270);
...
const metrics = {
  ...
  withinYearTerms: {
    count: withinYearResults.length,
    passCount: withinYearResults.filter((r: any) => r.status === "PASS").length,
    maxDiffHours: Math.max(...withinYearResults.map((r: any) => Math.abs(r.diffHours))),
    meanDiffHours: withinYearResults.reduce((sum: number, r: any) => sum + Math.abs(r.diffHours), 0) / withinYearResults.length,
  },
  crossYearTerms: {
    count: crossYearResults.length,
    passCount: crossYearResults.filter((r: any) => r.status === "PASS").length,
    note: "These terms naturally fall in the next calendar year; the engine's getSolarTermTimestamp searches within a single calendar year"
  },
  termSpacingCorrect: spacingOk,
  maxDiffHoursOverall: Math.max(...results.map(r => Math.abs(r.diffHours))),
  meanDiffHoursOverall: results.reduce((sum, r) => sum + Math.abs(r.diffHours), 0) / results.length,
  results
};
