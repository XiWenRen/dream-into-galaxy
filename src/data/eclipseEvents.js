...
/** 根据日期获取最近的日食事件 */
export function getNearestSolarEclipse(date) {
...
/** 根据日期获取最近的月食事件 */
export function getNearestLunarEclipse(date) {
    const t = date.getTime();
    let nearest = null;
    let minDiff = Infinity;
...
/** 获取指定时间窗口内的所有日食/月食 */
export function getEclipsesInRange(startDate, endDate) {
...
/** 格式化日食类型为中文 */
export function formatSolarEclipseType(type, lang) {
    const map = {
...
/** 格式化月食类型为中文 */
export function formatLunarEclipseType(type, lang) {
    const map = {
