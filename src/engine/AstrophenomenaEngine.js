...
import { SOLAR_ECLIPSE_EVENTS, LUNAR_ECLIPSE_EVENTS, formatSolarEclipseType, formatLunarEclipseType, } from '../data/eclipseEvents';
export const SYNODIC_MONTH_MS = 29.53059 * 24 * 60 * 60 * 1000;
export function getCurrentCycleNewMoon(referenceTimestamp) {
...
export function getExactMoonPhaseTime(baseNewMoon, phaseIndex) {
    if (phaseIndex === 0)
        return baseNewMoon;
    ...
    for (let iter = 0; iter < 4; iter++) {
        ...
        for (let offset = -5; offset <= 5; offset++) {
            ...
            const es = { x: -earthPos.x, y: -earthPos.y, z: -earthPos.z };
            // 在黄道面上投影计算相位角
            const angle = Math.atan2(es.x * moonRel.y - es.y * moonRel.x, es.x * moonRel.x + es.y * moonRel.y);
...
export class AstrophenomenaEngine {
     ...
     */
    static searchEclipseEvents(centerTimestamp, windowYears = 10) {
     ...
     * 获取某个日期的日食/月食详细描述
     */
    static getEclipseDescription(event, lang) {
        if (event.type === 'solar' && event.detail) {
            const se = event.detail;
     ...
     */
    static getEclipseWindow(timestamp, type) {
     ...
     * @returns 对应时间戳（毫秒）
     */
    static getSolarTermTimestamp(year, targetLongitude) {
        // 构造该年份的起止范围
        ...
        for (let i = 0; i < 50; i++) {
            ...
            if (Math.abs(diff) < 1e-6 || right - left < 1e-7)
                break;
        }
     ...
     * 查询当前处于哪个节气以及角度
     */
    static getCurrentSolarTerm(days) {
     ...
     */
    static computeShadowCone(lightSourceRadius, occluderRadius, distance) {
     ...
     */
    static getMoonShadowCone(sunToMoonDist, sunRadius = 696340, moonRadius = 1737.4) {
     ...
     */
    static getEarthShadowCone(sunToEarthDist, sunRadius = 696340, earthRadius = 6371) {
